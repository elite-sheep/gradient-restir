import argparse
from config import get_default_gpath_tracer_config, GPathTracerConfig
import os
import common
import falcor
import numpy as np
...
def render_gt(scene_file, reso: (int, int), max_bounces: int, num_iters: int, output_dir: str):
    ...
    img_primal = np.zeros(shape=(reso[1], reso[0], 3))
    img_dx = np.zeros(shape=(reso[1], reso[0], 3))
    img_dy = np.zeros(shape=(reso[1], reso[0], 3))

    for _ in tqdm.tqdm(range(num_iters)):
        testbed.frame()
        img_primal = img_primal + testbed.render_graph.get_output("GPathTracer.primal").to_numpy()[:,:,:3] / num_iters
        img_dx = img_dx + testbed.render_graph.get_output("GPathTracer.DX").to_numpy()[:,:,:3] / num_iters
        img_dy = img_dy + testbed.render_graph.get_output("GPathTracer.DY").to_numpy()[:,:,:3] / num_iters

    os.makedirs(output_dir, exist_ok=True)
    pyexr.write(f"{output_dir}/gt_primal.exr", img_primal)
    pyexr.write(f"{output_dir}/gt_dx.exr", img_dx)
    pyexr.write(f"{output_dir}/gt_dy.exr", img_dy)
...
def render_ref(scene_file, max_bounces: int, spatial_neighbour_search_radius: float, num_iters: int,
    ...
    for i in range(4):
        ...
        for _ in tqdm.tqdm(range(num_iters)):
            ...
            mse = np.mean(np.sqrt((cur_img[:,:-1,:3] / (i + 1) - gt_image[:,:-1,:3]) ** 2))
            mses[i] = mse / 4.0

        cur_img = testbed.render_graph.get_output("PathTracer.color").to_numpy()
    os.makedirs(output_dir, exist_ok=True)
    pyexr.write(f"{output_dir}/ref.exr", cur_img[:,:,:3])
    np.savetxt(f"{output_dir}/render_dx_ref.txt", np.array(mses))
...
def validate_primal(cfg: GPathTracerConfig):
    ...
    testbed = common.create_testbed(reso)
    _ = common.load_scene(testbed, cfg.scene_path, reso[0] / reso[1])
...
def render_dx(config: GPathTracerConfig):
    gt_image = pyexr.read(config.gt_path)
    reso = (gt_image.shape[1], gt_image.shape[0])
...
def render(config: GPathTracerConfig):
    reso = config.resolution
    primal_img = np.zeros(shape=(reso[1], reso[0], 3))
    dx_img = np.zeros(shape=(reso[1], reso[0], 3))
    dy_img = np.zeros(shape=(reso[1], reso[0], 3))
    recon_img = np.zeros(shape=(reso[1], reso[0], 3))
    os.makedirs(config.output_dir, exist_ok=True)

    primal_dir = os.path.join(config.output_dir, "primal")
    dx_dir = os.path.join(config.output_dir, "dx")
    dy_dir = os.path.join(config.output_dir, "dy")
    ...
    for i in range(config.num_images):
        ...
        for j in tqdm.tqdm(range(config.num_iters)):
            testbed.frame()
        primal_img = primal_img + testbed.render_graph.get_output("GPathTracer.primal").to_numpy()[:,:,:3] / config.num_images
        dx_img     = dx_img + testbed.render_graph.get_output("GPathTracer.DX").to_numpy()[:,:,:3] / config.num_images
        dy_img     = dy_img + testbed.render_graph.get_output("GPathTracer.DY").to_numpy()[:,:,:3] / config.num_images
        recon_img  = recon_img + testbed.render_graph.get_output("GPathTracer.color").to_numpy()[:,:,:3] / config.num_images

    recon_img[np.where(recon_img < 0.0)] = 0.0
    pyexr.write(f"{primal_dir}/primal.exr", primal_img)
    pyexr.write(f"{dx_dir}/dx.exr", dx_img)
    pyexr.write(f"{dy_dir}/dy.exr", dy_img)
    pyexr.write(f"{recon_dir}/recon.exr", recon_img)

def render_primal(config: GPathTracerConfig):
    gt_image = pyexr.read(config.gt_path)
    reso = (gt_image.shape[1], gt_image.shape[0])
...
def render_with_args(args):
    ...
    if gPathTracerConfig.type == 'dx':
        render_dx(gPathTracerConfig)
    elif gPathTracerConfig.type == 'primal':
        render_primal(gPathTracerConfig)
...
def gt_with_args(args):
    render_gt(os.path.abspath(args.scene), (args.w, args.h),
              args.max_bounces, args.spp,
              args.output_dir)

def ref_with_args(args):
    render_ref(os.path.abspath(args.scene), args.max_bounces,
              args.spatial_neighbour_search_radius, args.spp, args.gt,
              args.output_dir)
...
if __name__ == "__main__":
    ...
    parser_gt.add_argument('--spp', type=int, default=16384, help='Number of samples to render the ground truth')
    ...
    parser_ref.add_argument('--spatial_neighbour_search_radius', type=float, default=4.0, help='Spatial neighbour search radius')
