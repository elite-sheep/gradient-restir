...
def render_gt(scene_file, reso: (int, int), max_bounces: int, num_iters: int, output_dir: str):
    ...
    camera.position = camera_position + common.falcor.float3(0.002, 0.0, 0.0)
    camera.target = camera.target + common.falcor.float3(0.002, 0.0, 0.0)
    ...
    img = testbed.render_graph.get_output("AccumulatePass.output").to_numpy()
    ...
    pyexr.write(f"{output_dir}/dx_gt_dynamic.exr", dx[:,:,:3])
