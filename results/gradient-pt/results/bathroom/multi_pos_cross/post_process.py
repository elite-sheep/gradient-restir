...
for i in range(162):
    ...
    dx = img_remap(dx, -0.08, 0.08, os.path.join("./dx/", f"dx_{i}.png"))

    dy = read_exr(os.path.join("./dy/", f"dy_{i}.exr"))
    dy = img_remap(dy, -0.08, 0.08, os.path.join("./dy/", f"dy_{i}.png"))

    recon = read_exr(os.path.join("./recon_L2/", f"recon_{i}.exr"))
    recon = gamma_correction(recon, 2.2)
    save_png(recon, os.path.join("./recon_L2/", f"recon_{i}.png"))
